...
#if 0
    Serial.printf("Layer %d callback ========\n", mdl->layer_i);
    for(int y=0; y<h; y++){
        Serial.printf("[");
        for(int x=0; x<w; x++){
            Serial.printf("[");
            for(int c=0; c<ch; c++){
                ...
                Serial.printf("%.3f,", output[(y*w+x)*ch+c]);
#else
                Serial.printf("%.3f,", TML_DEQUANT(lh,output[(y*w+x)*ch+c]));
#endif
            }
            Serial.printf("],");
        }
...
static void parse_output(tm_mat_t* outs) {
  ...
  Serial.printf("### Predict output is: Class %d, Lable %s, prob %.3f\n", maxi, labels[maxi], maxp);
  return;
}
...
Serial.begin(115200);
  ...
  /* Begin TinyMaix */
  TM_DBGT_INIT();
  Serial.printf("mnist demo\n");
  tm_mdl_t mdl;

  // for (int i = 0; i < 28 * 28; i++) {
  //   Serial.printf("%3d,", pic[i]);
  //   if (i % 28 == 27)
  //     Serial.printf("\n");
  // }
  ...
  if (res != TM_OK) {
    Serial.printf("tm model load err %d\n", res);
    goto end;
  }
